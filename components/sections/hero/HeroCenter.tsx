...
export const HeroCenter: React.FC<HeroProps> = ({ section, onTextEdit, onImageClick, buttonClass, onElementSelect, onElementUpdate, selectedElementId, readOnly = false }) => {
  ...
  return (
    <div className={`${styles.maxWidth || 'max-w-5xl'} mx-auto px-6 text-center relative z-10`}>
      ...
      <p 
        className={`${subtitleFontWeightClass} ${subtitleAlignClass} opacity-80 mb-10 mx-auto max-w-2xl ${!readOnly ? 'outline-none focus:ring-2 ring-white rounded px-2' : ''} min-h-[1.5em] ${!readOnly && isSubtitleSelected ? 'ring-2 ring-blue-500 ring-offset-2 ring-offset-black z-20' : !readOnly ? 'hover:ring-1 hover:ring-white/20' : ''}`} 
        style={subtitleStyle}
        contentEditable={!readOnly}
        suppressContentEditableWarning={!readOnly}
